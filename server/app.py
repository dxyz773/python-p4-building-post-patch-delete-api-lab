...
def baked_goods():
    if request.method == "GET":
        ...
        serialized_goods = [goods.to_dict() for goods in all_baked_goods]
        ...
        response = make_response(dict_good, 201)
        return response
...
def baked_goods_by_id(id):
    ...
    if request.method == "GET":
        dict_good = baked_good.to_dict()
        response = make_response(dict_good, 200)
        return response
